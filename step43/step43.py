...
实现按模块拆在同一个包里，本文件只做两件事：转发公开 API、跑文本演示。
    ...
    python step43/step43.py                                  # 默认模型 + 默认问题
    python step43/step43.py --max-new-tokens 64 "问题一" "问题二"
    python step43/step43.py --model-dir /path/to/other --backend torch
"""
...
# 这个文件既可能被 `python step43/step43.py` 直接跑，也可能在 sys.path 指向
# step43/ 目录时被 `import step43` 命中。后一种情况下它挡住了同名包，让位给包。
_self = sys.modules.get("step43")
if _self is not None and not hasattr(_self, "__path__"):
    del sys.modules["step43"]
    import step43 as _package
