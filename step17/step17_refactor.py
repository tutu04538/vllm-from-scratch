    ...
    def step(self):
        ...
        with torch.inference_mode():
            ...
            def _forward_and_sample(scheduled_items):
                ...
                can_sample_idx = torch.tensor([i for i, item in enumerate(scheduled_items) if item["can_sample"]], device=self.model.device, dtype=torch.long)
                
                if can_sample_idx.shape[0] == 0:
                    return None
                
                last_token_positions = torch.tensor([item["num_scheduled_tokens"] - 1 for item in scheduled_items if item["can_sample"]], device=self.model.device, dtype=torch.long)
                logits = _logits[can_sample_idx, last_token_positions, :]
                output_ids = self.sampler.sample(logits)
                for i, item in enumerate(scheduled_items):
                    if item["can_sample"]:
                        item["request"].output_ids.append(output_ids[i].item())
                return None
