...
class KVCachePool:
    ...
    def __init__(self, block_size, num_kv_blocks, num_kv_heads, head_dim, device, enable_prefix_caching=True):
        self.block_size = block_size
        self.num_kv_blocks = num_kv_blocks
        self.num_kv_heads = num_kv_heads
        self.head_dim = head_dim
        self.device = device
        # 一个 slot 是一个 token 的缓存位置，一行里装这个 token 的全部 KV heads
        self.k_cache = torch.zeros(num_kv_blocks, block_size, num_kv_heads, head_dim, device=device)
        self.v_cache = torch.zeros(num_kv_blocks, block_size, num_kv_heads, head_dim, device=device)
        # 前两维看成一排 token 槽位，与底层存储共享，不是副本
        self.k_flat = self.k_cache.view(-1, num_kv_heads, head_dim)
        self.v_flat = self.v_cache.view(-1, num_kv_heads, head_dim)
...
class TinyCausalLM(nn.Module):
    ...
    def __init__(self, vocab_size=5, d_model=8, max_seq_len=32, device=None, attention_backend="torch",
        ...
        # Q 投影出 num_q_heads 个头；K/V 只投影出 num_kv_heads 个
        self.q_proj = nn.Linear(d_model, num_q_heads * self.head_dim, bias=False).to(self.device)
        self.k_proj = nn.Linear(d_model, num_kv_heads * self.head_dim, bias=False).to(self.device)
        self.v_proj = nn.Linear(d_model, num_kv_heads * self.head_dim, bias=False).to(self.device)

        # 各 head 输出按顺序拼接后由 o_proj 映射回 d_model
        self.o_proj = nn.Linear(d_model, d_model, bias=False).to(self.device)
    ...
    def _embeds_qkv(self, num_tokens):
        # 固定缓冲的前 num_tokens 行 -> 一次投影 -> 拆成 head
        inputs_embeds = self.token_embedding(self.input_buffer[:num_tokens]) + \
            self.position_embedding(self.position_buffer[:num_tokens])
        q = self.q_proj(inputs_embeds).view(num_tokens, self.num_q_heads, self.head_dim)
        k = self.k_proj(inputs_embeds).view(num_tokens, self.num_kv_heads, self.head_dim)
        v = self.v_proj(inputs_embeds).view(num_tokens, self.num_kv_heads, self.head_dim)
        return q, k, v
    ...
    def gpu_forward(self, num_tokens, kv_cache_pool):
        ...
        return self._heads_to_logits(out, num_tokens)

    def _heads_to_logits(self, out, num_tokens):
        # (N, num_q_heads, head_dim) -> 按 head 顺序拼回 (N, d_model) -> o_proj -> lm_head
        return self.lm_head(self.o_proj(out.reshape(num_tokens, self.d_model)))
    ...
    def _torch_forward(self, num_tokens, num_scheduled_tokens, past_kv, kv_cache_pool):
        ...
        for _cache, count in zip(past_kv, num_scheduled_tokens):
            ...
            heads_out[offset:offset + count] = self.block_attention(
                q[offset:offset + count], _cache, kv_cache_pool, query_positions)
            offset += count

        return self._heads_to_logits(heads_out, num_tokens)
    ...
    def block_attention(self, q, cache: CacheConfig, kv_cache_pool: KVCachePool, query_positions):
        ...
        for q_head in range(self.num_q_heads):
            ...
            for logical_block in range(num_blocks):
                ...
                k_block, v_block = kv_cache_pool.block_view(cache, logical_block, count)
...
class Engine:
    ...
    def __init__(self, max_num_seqs=1, max_num_batched_tokens=4,block_size=4, num_kv_blocks=8, vocab_size=5, d_model=8, max_seq_len=32, on_finished=None, enable_prefix_caching=True, device=None, attention_backend="torch", use_cuda_graph=False, num_q_heads=1, num_kv_heads=1):
        ...
        self.attention_backend = attention_backend
        self.enable_prefix_caching = enable_prefix_caching
        self.kv_cache_pool = KVCachePool(block_size, num_kv_blocks, self.model.num_kv_heads,
                                         self.model.head_dim, self.model.device, self.enable_prefix_caching)
