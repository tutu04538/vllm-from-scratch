...
def _check_request_ids(request, vocab_size):
    ...
    **返回物化后的 `(prompt_ids, max_new_tokens)`，调用方必须用这个返回值建请求。**
    `prompt_ids` 允许是任何可迭代对象，迭代器只能被消费一次：这里 `list(...)` 已经
    把它耗尽了，若调用方再去读 `request["prompt_ids"]`，拿到的是空序列——
    入队后 `prompt_ids=[]`，直到跑起来才炸（第五十二关验收记的那个洞）。
    """
...
class Scheduler:
    ...
    def add_request(self, request):
        ...
        # 校验函数顺手把 prompt_ids 物化：迭代器只能读一次，建请求必须用它的返回值，
        # 不能回头再读 request["prompt_ids"]（那样拿到的是已耗尽的空序列）
        prompt_ids, max_new_tokens = _check_request_ids(request, self.vocab_size)
        params = SamplingParams(vocab_size=self.vocab_size,
                                **{k: request[k] for k in SAMPLING_KEYS if k in request})
        # 第五十四关起投机支持任意采样参数：随机采样走拒绝采样（`verify_drafts_random`），
        # 惩罚项按**逐行历史**施加（见 docs/step54_random_speculative.md §2）。
        # 第五十二关那句话「只支持贪心且无惩罚项」已经不成立，限制随之删除。
        seq = SequenceConfig(request["request_id"], prompt_ids, max_new_tokens,
    ...
    def _plan_tokens(self):
        ...
        **真实 token 优先，草稿只吃余量**——这是本关批量投机的分配规则：

        1. 先给本轮每个 ready（只差最后一个 token）的请求各留 **1 个真实 token**。
           预算不够所有 ready 时按组内顺序保留靠前的，其余的这轮不运行；
        2. 剩下的额度才轮到草稿和 prefill，按遍历顺序先到先得；
        3. 所以预算 5、三条都 ready 时：先各留 1（共 3），余量 2 全给第一条的草稿，
           后面两条照样各算 1 个真实 token——草稿挤不掉别人的基础配额。
        ...
        for group in self._budget_groups():
            ...
            if len(ready_req) > token_budget_remaining:
                # 额度不够这么多 ready 请求：**真实 token 优先**——按组内顺序（fcfs 是
                # 到达序、priority 是同优先级内的到达序）保留靠前的各 1 个，其余的这轮
                # 不运行。这里以前断言「不会发生」，但 `max_num_seqs >
                # max_num_batched_tokens` 的配置就能造出来；断言不如按同一条规则处理，
                # 反正被丢下的那些走的是下面 `prefill_token_budget == 0` 那条路，
                # 不会排出 0 token 的计划项。
