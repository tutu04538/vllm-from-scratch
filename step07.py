...
class Engine:
    ...
    def step(self):
        ...
        for i, req in enumerate(self.running):
            ...
            last_token_id = prompt_ids[-1] if not current_result else current_result[-1]
            last_token_ids.append(last_token_id)
            
        output_ids = self._generate(last_token_ids)
    ...
    def _generate(self, last_token_ids):
        if type(last_token_ids) != torch.Tensor:
            last_token_ids = torch.tensor(last_token_ids, device=self.model.device)
        else:
            last_token_ids = last_token_ids.clone().to(self.model.device)
        
        logits = self.model.forward(last_token_ids)
