...
def read_raw_weights(model_dir, allowed_dtypes):
    """读外部权重文件，检查实际 dtype，再统一转成 FP32。

    两种格式的文件同名，所以读法只有一份；允许哪些 dtype 由适配器声明。
    """
    model_dir = pathlib.Path(model_dir)
    ...
    # 文件精度不等于运行精度：进来之后统一 FP32，模型、KV 缓存和计算全程 FP32
    return {name: tensor if tensor.dtype == torch.float32 else tensor.float()
            for name, tensor in weights.items()}
