...
class TinyCausalLM(nn.Module):
    ...
    def _attention_out(self, layer, layer_idx, hidden, num_tokens, kv_cache_pool):
        # 一层里的 Attention 段：QKV -> 写本层 KV -> 分页 attention -> o_proj
        q, k, v = self._layer_qkv(layer, layer.norm1(hidden), num_tokens)
    ...
    def gpu_forward(self, num_tokens, kv_cache_pool):
        ...
        for layer_idx, layer in enumerate(self.layers):
            h = hidden + self._attention_out(layer, layer_idx, hidden, num_tokens, kv_cache_pool)
...
class Engine:
    ...
    def __init__(self, max_num_seqs=1, max_num_batched_tokens=4,block_size=4, num_kv_blocks=8, vocab_size=5, d_model=8, max_seq_len=32, on_finished=None, enable_prefix_caching=True, device=None, attention_backend="torch", use_cuda_graph=False, num_q_heads=1, num_kv_heads=1,
                 num_layers=2, intermediate_size=64, rms_norm_eps=1e-6):
