"""逐步对比 step49 与 step50：证明第五十关只换了「怎么找淘汰对象」。

同一模型 seed、同一请求、同一动态到达时刻，逐步比较：
  running / waiting 顺序、本轮计划、输入 token、完成输出、每请求计数、
  承诺总额、块引用与双向 hash。

物理块**编号不再要求一致**（用户明确放开：只测功能）。step50 按 vLLM 的做法，
位置即优先级：无 hash 的空闲块在队首（优先复用），带 hash 的闲置缓存在队尾（最后淘汰）。
...
from step49 import Engine as Engine49
...
def snapshot(e):
    ...
    return {
        ...
        # 物理块编号**不再比较**：step50 改成 vLLM 式的单链表索引，
        # 无 hash 的空闲块前插（优先复用）、带 hash 的闲置缓存后插（最后淘汰），
        # 淘汰顺序是「释放先后」而不是 step49 的精确 (block_last_used, b)。
        # 用哪一块不影响正确性；「不会读到错误 KV」由既有回归的 KV 数值对照覆盖。
    }


def run_pair(seed, arrival_plan, limit=800, **kw):
    engines = [build(Engine49, seed, **kw), build(Engine50, seed, **kw)]
    for e in engines:
...
for label, kw, plan in SCENARIOS:
    for seed in (29, 7):
        ...
        t49, t50 = traces
        diff = [(i, a, b) for i, (a, b) in enumerate(zip(t49, t50), 1) if a != b]
        check(f"{label}（seed={seed}）：逐步计划 / 队列 / 输出完全一致",
              not diff and len(t49) == len(t50),
        ...
        s49, s50 = summarize(engines[0]), summarize(engines[1])
        # cached_blocks_kept 会因淘汰选择不同而不同（留下的可复用块数），
        # 那不是功能差异，从比对里去掉；其余字段必须一致。
        a, b = dict(s49), dict(s50)
        a.pop("cached_blocks_kept"), b.pop("cached_blocks_kept")
        check(f"{label}（seed={seed}）：结束时状态一致", a == b,
              "" if a == b else f"\n  49={a}\n  50={b}")
        check(f"{label}（seed={seed}）：引用与承诺归零、空闲堆与真实空闲一致",
              s50["promised_pool"] == 0 and s50["usage_zero"] and s50["hash_consistent"]
              and s49["running_empty"]
              and set(_alloc_chain(engines[1].kv_cache_pool))
              == set(engines[1].kv_cache_pool._allocatable_block_indices()), str(s50))
