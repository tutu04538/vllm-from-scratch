...
def wall_samples(fn,n=31):
    ...
    for _ in range(n):
        torch.cuda.synchronize();start=time.perf_counter_ns();fn();torch.cuda.synchronize()
        samples.append((time.perf_counter_ns()-start)/1000)
    return stats(samples,'CPU wall clock from call through final CUDA synchronize; 8 warmups, 31 samples; input preparation inside fn included, first compile excluded')


def graph_samples(fn):
    # Only the measurement harness captures; the student's Engine does NOT implement CUDA Graph.
    for _ in range(8):fn()
    torch.cuda.synchronize()
    graph=torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph):captured=fn()
    ...
    return stats(samples,'Pre-resident inputs, measurement-harness-only CUDA Graph replay, CUDA Event elapsed time. Excludes Python dispatch/metadata creation/H2D/first compile and capture. Same capture method for both backends; not production Engine latency.'),captured
...
def main():
    ...
    report=dict(kind=args.kind,backend=args.backend,workload=args.workload,source_sha256=digest,script_sha256=sha(Path(__file__)),
