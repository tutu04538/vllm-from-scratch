"""第十三关完整 Engine 生命周期对照；CPU 同权重，源文件不变，调试 print 临时关闭。"""
import os
...
FIXTURES = {"step09": RECORDS / "step09_contract_20260915T090359.213207Z.json",
            "step13": RECORDS / "step13_contract_20260915T090330.639923Z.json"}
REQUESTS = [
...
def main():
    ...
    parser.add_argument("--module", choices=["step09", "step13"], default="step13")
    ...
    case_name = ("random_capacity_" if args.module == "step13" else "unequal_lengths_capacity_") + str(args.capacity)
    fixture = next(c for c in accepted["cases"] if c["name"] == case_name)
    wanted_calls = ([{"kind":c["kind"], "ids":c["ids"]} for c in fixture["calls"]] if args.module == "step13"
                    else [{"kind":"forward", "ids":c["ids"]} for c in fixture["forwards"]])
    ...
    def check():
        ...
        original_init, original_forward = module.TinyCausalLM.__init__, module.TinyCausalLM.forward
        ...
        def forward(model,ids,*a,**kw):
            return observe("prefill" if args.module == "step13" else "forward",original_forward,model,ids,*a,**kw)
        ...
        with ExitStack() as stack:
            for obj,name,fn in ((module.TinyCausalLM,"__init__",init),(module.TinyCausalLM,"forward",forward),
                                (module.Sampler,"sample",sample),(module.Engine,"step",step)):
                stack.enter_context(patch.object(obj,name,fn))
            if args.module == "step13":
    ...
    path = directory / f"step13_compare_{args.module}_cap{args.capacity}_{stamp}_{os.getpid()}.json"
