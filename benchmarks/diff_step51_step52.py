"""投机关关闭时，对照 step51 的 recompute 模式与 step52。

step52 已删掉 preemption_mode 与承诺账本；其默认容量策略不再等价于
step51 的默认承诺式准入。build() 因此为 Engine51 显式设置
preemption_mode="recompute"，再逐步比较调度、输出、块表与请求计数。
这里验证的是上述两种容量策略的等价性，不是旧默认行为保持不变。
