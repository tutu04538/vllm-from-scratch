...
from step33_precision_reference import reference as precision_reference
from step33_source import source_digest,source_files
from step33_test_helpers import m
from step33_external_process import mapped
from step33.step33 import encode
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--mode',required=True,choices=['torch','eager','graph']);ap.add_argument('--dtype',choices=['float32','bfloat16'],required=True);ap.add_argument('--norm-backend',choices=['torch','triton'],required=True);ap.add_argument('--repeats',type=int,default=5);a=ap.parse_args();dt=getattr(torch,a.dtype)
