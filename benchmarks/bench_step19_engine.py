"""step18/19 paired comparison: unequal prefill, mixed arrival, warm mixed arrival.
...
def main():
    ...
    parser.add_argument('--mode',choices=['step18','step19'],required=True)
    ...
    helper=importlib.import_module('step19_test_helpers' if args.mode=='step19' else 'verify_step18_without_lru_initial')
    fixtures={}
    ...
    def check():
        ...
        while e.has_unfinished_requests():
            ...
            assert len(row['calls'])<=(1 if args.mode=='step19' else 2)
        ...
        projected=sum(len(c['inputs']) if args.mode=='step19' else len(c['inputs'])*len(c['inputs'][0]) for c in calls)
        ...
        if args.mode=='step19':assert projected==true
    ...
    path=PROJECT/'benchmarks/results'/('step19_compare_'+args.workload+'_'+args.mode+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')+'_'+str(os.getpid())+'.json')
