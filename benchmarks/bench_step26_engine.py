...
from step26_test_helpers import m,engine_for,state_for,reference
...
def main():
    ...
    report_path=sorted(RECORDS.glob('step26_contract_*.json'))[-1];report=json.loads(report_path.read_text())
    ...
    data=dict(source_sha256=digest,script_sha256=sha(Path(__file__)),helper_sha256=sha(RECORDS/'tools/step26_test_helpers.py'),contract_report=str(report_path),mode=args.mode,
        status='verified_multilayer_engine',device=torch.cuda.get_device_name(),torch=torch.__version__,triton=triton.__version__,dtype='float32',tf32=False,threads=1,
        model=dict(d_model=32,num_q_heads=4,num_kv_heads=2,head_dim=8,num_layers=2,intermediate_size=64,rms_norm_eps=1e-6),weight_hash=state_hash(state),requests=reqs,results=result,steps=steps,graph_keys=sorted(graphs),
    ...
    path=ROOT/'benchmarks/results'/('step26_engine_'+args.mode+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')+'.json');path.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
