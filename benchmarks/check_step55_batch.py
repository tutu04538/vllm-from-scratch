"""第 55 关的回归套件：第五十三关的行映射 / 混批 / 预算 / 容量 / 恢复。
...
import step55
from step55 import Engine as Engine53
from step55.sample_runtime import SampleRuntime
...
A, B, C, D = (step55.SequenceConfig(rid, [1], 4, 4) for rid in "ABCD")
manual = [
...
plans = []
_original_plan_tokens = step55.scheduler.Scheduler._plan_tokens
...
step55.scheduler.Scheduler._plan_tokens = _spy
try:
    pressure = build(3, None, **PRESSURE_ARGS)
    pressure_steps = run(pressure, None, arrivals={0: [A_REQ, B_REQ]})
finally:
    step55.scheduler.Scheduler._plan_tokens = _original_plan_tokens
