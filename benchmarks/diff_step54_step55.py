"""投机关关闭时，step55 与 step54 应当逐步一致。

第五十五关新增的是 `speculative_mode="draft_model"`（双模型 + 双 KV），
None 与 ngram 的既有路径**不动**。所以 `speculative_mode=None` 下两者必须逐字节
一致——包括物理块编号、hash 链、完成顺序与每个请求的计数。这里不放宽任何字段。
...
from step54 import Engine as Engine54
from step55 import Engine as Engine55
...
def run_pair(seed, arrival_plan, limit=800, **kw):
    engines = [build(Engine54, seed, **kw), build(Engine55, seed, **kw)]
    for e in engines:
