...
from step28_test_helpers import m,engine_for,state_for,reference
...
def main():
    ...
    sha=digest(Path(m.__file__));core=sorted(RECORDS.glob('step28_contract_*.json'))[-1]
    cr=json.loads(core.read_text());assert cr['source_sha256']==sha and cr['status']=='passed'
    torch.manual_seed(17);state=state_for(4,2)
    ...
    with tempfile.TemporaryDirectory(prefix='step28_bench_') as tmp,torch.inference_mode():
        directory=Path(tmp)/'model';src=engine_for(4,2,'cpu',enable_prefix_caching=False);src.model.load_state_dict(state);m.save_model(src.model,directory);del src
