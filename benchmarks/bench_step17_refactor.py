"""第十七关重构对照：原版与统一 forward/分组采样版，都使用预算4。
...
FIXTURES={'step16':'step16_engine_contract_20260916T071055.671479Z.json',
          ...
          'step17_refactor':'step17_refactor_engine_contract_20260916T113523.753028Z.json'}
...
def main():
    ...
    parser.add_argument('--mode',choices=['step17_chunk4','step17_refactor'],required=True)
    parser.add_argument('--workload',choices=['static','arrival'],required=True)
    args=parser.parse_args();torch.set_num_threads(1)
    name='step17_refactor' if args.mode=='step17_refactor' else 'step17'
    refactored=name=='step17_refactor'
    ...
    def check(e):
        ...
        current_group=[]
        if refactored:
            append=e.model._forward_append
        else:
            pre,dec=e.model.forward_prefill,e.model.forward_decode
        def observe(kind,fn,ids,*a,**kw):
            ...
            return fn(ids,*a,**kw)
        def observe_append(ids,counts,caches,pool):
            nonlocal current_group
            current_group=[next(s for s in e.running if s.cache is c) for c in caches]
            kind='decode' if all(s.output_ids for s in current_group) else 'prefill'
            calls.append(dict(step=step_index,kind=kind,ids=ids.tolist(),real_lengths=list(counts),
                              true_tokens=sum(counts),padded_positions=ids.numel()))
            assert torch.is_inference_mode_enabled() and not e.model.training
            return append(ids,counts,caches,pool)
        def sampling(logits):
            ready=[s for s in (current_group if refactored else e.running) if s.cache.length>=len(s.prompt_ids)]
            assert ready, 'empty sampling' 
        ...
        with ExitStack() as stack:
            if refactored:
                stack.enter_context(patch.object(e.model,'_forward_append',side_effect=observe_append))
            else:
                stack.enter_context(patch.object(e.model,'forward_prefill',side_effect=lambda ids,*a,**kw:observe('prefill',pre,ids,*a,**kw)))
                stack.enter_context(patch.object(e.model,'forward_decode',side_effect=lambda ids,*a,**kw:observe('decode',dec,ids,*a,**kw)))
            stack.enter_context(patch.object(e.sampler,'sample',side_effect=sampling))
    ...
    path=PROJECT/'benchmarks/results'/f"step17_refactor_compare_{args.workload}_{args.mode}_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')}_{os.getpid()}.json"
