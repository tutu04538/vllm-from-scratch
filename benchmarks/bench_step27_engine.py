...
from step27_test_helpers import m,engine_for,state_for,reference
...
def main():
    ...
    report_path=sorted(RECORDS.glob('step27_contract_*.json'))[-1];report=json.loads(report_path.read_text())
    ...
    data=dict(source_sha256=digest,script_sha256=sha(Path(__file__)),helper_sha256=sha(RECORDS/'tools/step27_test_helpers.py'),contract_report=str(report_path),mode=args.mode,
        status='verified_rope_engine',device=torch.cuda.get_device_name(),torch=torch.__version__,triton=triton.__version__,dtype='float32',tf32=False,threads=1,
        model=dict(d_model=32,num_q_heads=4,num_kv_heads=2,head_dim=8,num_layers=2,intermediate_size=64,rms_norm_eps=1e-6,rope_theta=10000.),weight_hash=state_hash(state),requests=reqs,results=result,steps=steps,graph_keys=sorted(graphs),
        ...
        scope='Reused Engine, prefix caching disabled, same weights/requests. Submission + scheduling + packed forward + sampling + release + results + synchronization. 16 warmups, 31 groups of five complete workloads. Excludes model/Engine init, first JIT/library setup and graph capture. Full RoPE contract passed, including range validation before mutation. No timing of invalid requests.')
    path=ROOT/'benchmarks/results'/('step27_engine_'+args.mode+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')+'.json');path.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
