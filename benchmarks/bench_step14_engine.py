"""第十四关额外 Engine 完整 Engine 生命周期对照；CPU 同权重，源文件不变，调试 print 临时关闭。"""
import os
...
FIXTURES = {"step13": RECORDS / "step13_contract_20260915T090330.639923Z.json",
            "step14": RECORDS / "step14_engine_contract_20260915T111003.407774Z.json"}
REQUESTS = [
...
def main():
    ...
    parser.add_argument("--module", choices=["step13", "step14"], default="step14")
    ...
    def check():
        ...
        forward_name = "forward_prefill" if args.module == "step14" else "forward"
        ...
        with ExitStack() as stack:
            ...
            if args.module in ("step13", "step14"):
    ...
    path = directory / f"step14_engine_compare_{args.module}_cap{args.capacity}_{stamp}_{os.getpid()}.json"
