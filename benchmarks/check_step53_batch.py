"""第 53 关：批量投机验证——行映射、提交顺序、预算与容量压力。
...
def build(seed, script, **cfg):
    ...
    inner = step53.TinyCausalLM(device="cpu", attention_backend="torch",
...
A, B, C, D = (step53.SequenceConfig(rid, [1], 4, 4) for rid in "ABCD")
manual = [
...
plans = []
_original_plan_tokens = step53.scheduler.Scheduler._plan_tokens
...
step53.scheduler.Scheduler._plan_tokens = _spy
try:
    pressure = build(3, None, **PRESSURE_ARGS)
    pressure_steps = run(pressure, None, arrivals={0: [A_REQ, B_REQ]})
finally:
    step53.scheduler.Scheduler._plan_tokens = _original_plan_tokens
