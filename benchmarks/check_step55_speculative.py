"""第 55 关的回归套件（第五十二关的纯函数与配置校验 + 本关的 draft_model 校验）。
...
from step55 import Engine, KVCachePool, SequenceConfig
from step55.speculative import propose_ngram, verify_drafts
...
for label, request in [
    ...
    ("greedy + 三种惩罚", dict(repetition_penalty=1.2, presence_penalty=0.5,
                              frequency_penalty=0.2)),
    ("随机 + top-k/top-p + 惩罚", dict(temperature=0.8, top_k=20, top_p=0.9,
                                      frequency_penalty=0.3, seed=7)),
]:
    engine = Engine(**BASE)
    check(f"投机模式下 {label}：可以入队（本关起支持）",
          raises(engine.add_request,
                 dict(request_id="x", prompt_ids=[1, 2, 3], max_new_tokens=4, **request)) is None)
...
from step55.scheduler import Scheduler
