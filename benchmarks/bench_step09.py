"""第九关 CPU 完整 Engine 生命周期；固定随机初始化，三请求九个新 token。"""
import os
...
FIXTURE = Path("/home/user/proj/vllm-omni/learning_notes/14_vllm_from_scratch/验收记录/step09_contract_20260914T103446.129269Z.json")
REQUESTS = [
...
def run_static(capacity):
    ...
    engine = step09.Engine(max_num_seqs=capacity, vocab_size=5, d_model=8, max_seq_len=32)
...
def check_workload(capacity, fixture, canonical_weight_hash):
    ...
    original_init = step09.TinyCausalLM.__init__
    original_forward = step09.TinyCausalLM.forward
    original_sample = step09.Sampler.sample
    original_step = step09.Engine.step
    ...
    def forward(model, ids):
        ...
        result = original_forward(model, ids)
    ...
    with patch.object(step09.TinyCausalLM, "__init__", init), \
         patch.object(step09.TinyCausalLM, "forward", forward), \
         patch.object(step09.Sampler, "sample", sample), \
         patch.object(step09.Engine, "step", step):
...
def main():
    ...
    source = PROJECT / "step09" / f"step09.py"
    digest = hashlib.sha256(source.read_bytes()).hexdigest()
    canonical_weight_hash = parameter_hash(step09.TinyCausalLM(vocab_size=5, d_model=8, max_seq_len=32))
    ...
    report = {
        ...
        "boundary": "restore CPU RNG state + new Engine/model (random initialization and eval) + submit + query/step loop (including padding, inference_mode entry, forward, sample, writeback) + collect results; excludes imports, input fixtures, reference/tracing/checks",
        "comparison_scope": "new real-model Engine lifecycle baseline, not comparable with step08 forward-only or step07 dummy-model workload",
        "min_run_time_s": 1.0,
    }
    try:
        ...
        measurement = Timer(stmt="run_static(capacity)", globals={"run_static": run_static, "capacity": args.capacity},
                            num_threads=1, label="step09: CPU real-model Engine lifecycle",
    ...
    target = directory / f"step09_cap{args.capacity}_{stamp}_{os.getpid()}.json"
