...
import argparse,copy,hashlib,json,statistics,sys,time,importlib,math
...
def attention(args):
    ...
    if args.mode=='step23':
        model.attention_metadata=m.AttentionMetadata(len(f['lengths']),max(32,sum(f['counts'])),math.ceil(max(32,max(f['lengths']))/f['size']),torch.device('cuda'))
...
def main():
    global m,attention_fixture,observe_step
    parser=argparse.ArgumentParser();parser.add_argument('--kind',choices=['attention','engine'],required=True)
    parser.add_argument('--mode',choices=['step22','step23'],required=True);parser.add_argument('--backend',choices=['triton'],default='triton');parser.add_argument('--workload',required=True)
    args=parser.parse_args();torch.set_num_threads(1);torch.backends.cuda.matmul.allow_tf32=False
    helper=importlib.import_module(args.mode+'_gpu_helpers')
    m=helper.m;attention_fixture=helper.attention_fixture;observe_step=helper.observe_step
    source=Path(m.__file__);digest=sha(source)
    fixtures={}
    for kind in ['gpu','model','prefix','disabled_engine','slots','attention']+(['metadata'] if args.mode=='step23' else []):
        path=sorted(RECORDS.glob(args.mode+'_'+kind+'_contract_*.json'))[-1];d=json.loads(path.read_text())
    ...
    report=dict(mode=args.mode,measurement_revision='batched_v2',kind=args.kind,backend=args.backend,workload=args.workload,source_sha256=digest,script_sha256=sha(Path(__file__)),
                helper_sha256=sha(RECORDS/('tools/'+args.mode+'_gpu_helpers.py')),fixtures=fixtures,device=torch.cuda.get_device_name(0),
                torch=torch.__version__,cuda=torch.version.cuda,triton=triton.__version__,dtype='float32',threads=1,tf32=False,**result)
    path=PROJECT/'benchmarks/results'/('step23_compare_'+args.mode+'_'+args.kind+'_'+args.workload+'_'+args.backend+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')+'_'+str(os.getpid())+'.json')
    path.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
    print(args.mode,args.kind,args.workload,args.backend,{k:round(v['median_us'],3) for k,v in result['timing'].items()},path,flush=True)
