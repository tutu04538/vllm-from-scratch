...
def generate_many(requests, on_finished=None):
    results = []
    done = set()
    ...
    while True:
        for i, (req, result) in enumerate(zip(requests, results)):
            ...
            if output_id.item() == 4 or len(results[i]["output_ids"]) >= max_new_tokens:
                if on_finished:
                    on_finished(results[i])
                done.add(request_id)
...
if __name__ == "__main__":
    ...
    results = generate_many(requests, on_finished=on_finished)
