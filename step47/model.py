...
class TinyCausalLM(nn.Module):
    ...
    def _layer_qkv(self, layer, hidden, num_tokens, positions):
        ...
        #
        # 一次 GEMM 算出 QKV，再沿最后一维切开。切片是**视图**：N=1 时它恰好紧凑，
        # N>1 时最后一维的 stride 是 (HQ+2*HKV)*head_dim 而不是 HQ*head_dim。
        # 两条下游都按传入的 stride 寻址（rope.py 与 attention 的 kernel 都收 stride），
        ...
        k = qkv[:, nq:nq + nk].view(num_tokens, self.num_kv_heads, self.head_dim)
        v = qkv[:, nq + nk:].view(num_tokens, self.num_kv_heads, self.head_dim)
        if self.use_qk_norm:
