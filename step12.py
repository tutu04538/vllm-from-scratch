...
class TinyCausalLM(nn.Module):
    ...
    def forward_decode(self, input_ids: torch.Tensor, past_kv):
        ...
        token_embeds = self.token_embedding(input_ids)
        position_ids = torch.tensor([[past_k.shape[1]] for past_k, _ in past_kv], device=input_ids.device)
        position_embeds = self.position_embedding(position_ids)
        ...
        max_seq_len = max(past_k.shape[1] for past_k, _ in past_kv) + 1
        
        new_past_kv = []
        for i in range(len(past_kv)):
            new_past_kv.append((torch.cat((past_kv[i][0], k[i:i+1]), dim=1), torch.cat((past_kv[i][1], v[i:i+1]), dim=1)))
        
        new_k = torch.cat([torch.cat((new_past_k, torch.zeros(new_past_k.shape[0], max_seq_len - new_past_k.shape[1], new_past_k.shape[2], device=new_past_k.device)), dim=1) for new_past_k, _ in new_past_kv], dim=0)
        new_v = torch.cat([torch.cat((new_past_v, torch.zeros(new_past_v.shape[0], max_seq_len - new_past_v.shape[1], new_past_v.shape[2], device=new_past_v.device)), dim=1) for _, new_past_v in new_past_kv], dim=0)
...
class Engine:
    ...
    def step(self):
        ...
        for i, req in enumerate(self.running):
            ...
            if past_kv is None:
                ...
                input_ids.append([current_result[-1]])
        ...
        if past_kv is None:
            ...
            attention_mask = [[1] for _ in input_ids]  # Only the last token is attended to when past_kv is available
    
        output_ids, past_kv = self._generate(input_ids, attention_mask, past_kv)
    ...
    def _generate(self, input_ids, attention_mask, past_kv):
        ...
        with torch.inference_mode():
            logits, past_kv = self.model.forward(input_ids, past_kv=past_kv)
            next_token_id = self.sampler.sample(logits[:, -1, :])
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        _, cache_a = model(torch.tensor([[0, 1, 2]], device=model.device))
        _, cache_b = model(torch.tensor([[3]], device=model.device))
        
        print(cache_a[0].shape, cache_a[1].shape)  # (1, 3, 8), (1, 3, 8)
        print(cache_b[0].shape, cache_b[1].shape)  # (1, 1, 8), (1, 1, 8)

        # 此处 1、2 是固定测试输入，不要求先写生成循环。
        logits, caches = model.forward_decode(
            torch.tensor([[1], [2]], device=model.device),
            [cache_a, cache_b],
        )
        
        print(logits)
        print(logits.shape)  # (2, 1, 5)
        print(caches[0][0].shape, caches[0][1].shape)  # (1, 4, 8), (1, 4, 8)
