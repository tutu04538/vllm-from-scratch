...
class Engine:
    ...
    def step(self):
        ...
        with torch.inference_mode():
            ...
            for i, req in enumerate(self.running):
                ...
                input_ids = torch.tensor([req.prompt_ids], device=self.model.device)
                attention_mask = torch.tensor([[1] * len(input_ids[0])], device=self.model.device)
                _logits, _past_kv = self._generate(input_ids, attention_mask=attention_mask, past_kv=None)
                logits[i] = _logits[:, -1, :]
                new_past_kv[i] = _past_kv[0]  # Store the new past_kv for this sequence
            ...
            attention_mask = [[1] for _ in decode_input_ids]  # Only the last token is attended to when past_kv is available
            decode_input_ids = torch.tensor(decode_input_ids, device=self.model.device)
            print("decode_input_ids:", decode_input_ids)
            if decode_input_ids.shape[0] > 0:
    ...
    def _generate(self, input_ids, attention_mask, past_kv):
        ...
        with torch.inference_mode():
            logits, past_kv = self.model.forward(input_ids)
            return logits, past_kv
