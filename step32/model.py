...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, num_q_heads, num_kv_heads, head_dim, intermediate_size, eps, use_qk_norm=False):
        super().__init__()
...
class TinyCausalLM(nn.Module):
    ...
    def __init__(self, vocab_size=5, d_model=8, max_seq_len=32, device=None, attention_backend="torch",
                 ...
                 eos_token_ids=None, dtype=torch.float32):
        super().__init__()
        ...
        self.layers = nn.ModuleList([
            DecoderLayer(d_model, num_q_heads, num_kv_heads, head_dim, intermediate_size,
                         rms_norm_eps, use_qk_norm)
            for _ in range(num_layers)
        ]).to(device=self.device, dtype=dtype)

        self.norm = RMSNorm(d_model, rms_norm_eps).to(device=self.device, dtype=dtype)   # 最终 norm
