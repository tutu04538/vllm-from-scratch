...
- 包摘要 SHA256：`6196aa0d6596d5d6…`（16 个 .py / 3794 行，验收方 `source_digest()` 口径）
- 基线：`step52/`，指纹 `6e3be2b00b03e2a4…`（16 个 .py / 3753 行），原样保留未改
- **改动 4 个文件**：
...
1. **投机开着时不需要走采样后端**：`add_request()` 已经保证所有请求都是贪心且无惩罚项，
   ...
   会为每条请求付一次同步。
2. **`.to(torch.float32)` 不能省**：BF16 下两个在 FP32 里不等的 logits 可能并成同一个值，
   argmax 的结果就会和普通路径不一致——而「投机不改变结果」正是本关要守的。
