...
```text
逐位置（0..K-1）：
    p[d] == 0  -> 必拒，不抽 uniform，本轮结束
    p[d] == 1  -> 必接受，不抽 uniform
    否则        -> 抽 1 个 uniform，决定接受还是拒绝
    接受之后**立刻**判终止：
        这一枚是终止 token -> 当场结束：后面的草稿不验证、bonus 不抽、一个随机数都不再消耗
没在终止 token 上停下时才继续：
    首次拒绝        -> 抽 1 次纠正 token，本轮结束
    全部接受        -> 抽 1 次 bonus
```

终止检查的位置是**约定的核心**，不是实现细节：它必须在「接受」的**当下**发生，而不是
「先把整个草稿前缀验证完、再回头截断输出」。后者的文本结果（提交的 token、保留的 KV）
一模一样，但**随机流会错位**——多抽的那几次 uniform 会让该请求后续每一步的随机数与
「没有投机时」对不上。两条接受分支（`p[d]=1` 的必接受、抽 uniform 的正常接受）都要
经过这条检查，必接受那条不是跳过它的捷径。反例与修法见 §8.10。
...
### 6.1 验证层（`benchmarks/check_step54_rejection.py`，36 项全通过）

- **确定性分支**（注入 `draw_uniform` / `draw_token`）：全接受、首拒绝、部分接受、bonus、
  草稿 EOS（必接受与抽 uniform **两条分支各一条**）、bonus EOS、纠正 EOS、K=0；
  每一条都断言**提交的 token、保留的 KV 数、随机数调用次数**；
...
### 8.10 验收复验指出的漏子：接受 EOS 之后还在消费随机数

**验收结论**：第五十四关**暂未通过**（`验收记录/183_第五十四关验收_接受EOS后仍消费随机数.md`）。
文本、KV 回滚、分布统计都正确，但「接受终止 token 之后不再消费随机数」这条约定没实现。

反例（EOS=2）：草稿 `[2,1]`、行 0 的 `p(EOS)=0.5`、uniform `0.1`。
...
| uniform 调用 | 1 | **2** |
| `num_accepted` | 1 | **2** |
| 请求自己的 `torch.Generator` 状态 | 消耗 1 次 | **消耗 2 次** |

草稿 `[0,2,1]`、EOS 在中间的那条也一样：应当 2 次，实际 3 次。文本看不出来，随机流错位了。

**根因**：`verify_drafts_random()` 的时序是

```text
验证整个草稿前缀（循环） -> 循环结束 -> 查找已接受的 EOS -> 截断输出
```

EOS 是**事后**查出来的，所以它后面的草稿照样被验证、照样抽 uniform。修法是把检查挪进
循环、放在**接受之后**：

```text
验证当前草稿 -> 接受则计数 -> 若它是终止 token，当场返回停止结果 -> 否则继续下一枚
```

两条接受分支（`p[d]=1` 的必接受、抽 uniform 的正常接受）都经过这条检查——只改后者、
让前者的 `continue` 跳过它，是这次验收里最容易犯的第二种错。修完 `num_accepted` 只统计
**截止 EOS 真正验证接受**的草稿（含 EOS 本身），EOS 后面的内容不再读取。回滚与 EOS 截断
仍交给 `_finish_candidates()`，没有第二份实现。

**为什么我原来的测试没抓住**：`check_step54_rejection.py` 那条「草稿本身是 EOS」的用例，
让 EOS **后面那枚草稿的概率也是 1**——代码虽然继续验证它，却因为 `p=1` 不抽 uniform，
随机数计数仍是 0，于是漏了。更糟的是那条用例把 `num_accepted=2` 写成了期待值，等于把
「先验证后截断」这个错误行为固定下来。**这次改的是两处**：实现，以及这条用例的语义
（期待值改成 1，并把后面那枚草稿的概率改成 0.4，让多抽的那次 uniform 暴露出来），
另加一条走 **uniform 接受分支**碰上 EOS 的用例（`[1,3,2]`，第三枚概率 0.4）、
以及一条「全部接受且 **bonus 是 EOS**」的用例（它和「草稿是 EOS」的区别正在于
bonus 那次抽样照常发生、草稿的 KV 一个都不用退）。

**验证**：

| 检查 | 结果 |
|---|---|
| 验收方的 `benchmarks/review_step54_eos_rng.py`（两条反例，含真实 `Generator` 状态比对） | PASS |
| 同脚本在**未修**的实现上跑 | 两条都 FAIL（uniform 2 vs 1、3 vs 2，`num_accepted` 与 generator 状态都错） |
| 我新增/修正的 4 条断言在未修的实现上跑 | 4 条全 FAIL，即它们确实有判别力 |
| `check_step54_rejection.py` | 32 → **36** 项（修正 1 条语义 + 新增 4 条） |
| 七个脚本（含 `check_step54_rejection.py` 34 项）与 `diff_step53_step54.py` 88 项 | 全过 |
| 验收方的 `review_step53.py` 重跑（68 组普通/投机对照 + 12 组采样/惩罚/Graph 差分） | 全过 |

**没改的地方**：`verify_drafts()`（贪心路径）不受影响——它用 `_finish_candidates()`
做 EOS 截断，本来就不消费随机数；`kept_inputs` 与 `committed_ids` 在修复前后完全相同，
变的只有 `num_accepted` 与随机数消耗，所以引擎侧的 KV 回滚逻辑一行没动。
