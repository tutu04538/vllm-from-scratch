...
4. 保留的输入 = `1 + 还需要当下一轮输入的草稿数`：
   ...
   - **被接受的草稿本身若是终止 token，它也不留**——它结束了这条请求，
     不会再作为下一轮输入，所以要留的是它**之前**那些草稿。

   最后一条在代码里是 `kept_drafts = index`（`index` 是终止 token 在候选里的位置），
   没有分支——两种终止位置的结果恰好都是 `index`：

   | 终止的是 | `index` | 要留几枚 | 为什么 |
   |---|---|---|---|
   | 草稿 `d_index`（`index < num_accepted`） | `index` | `index` | 它的 KV 要退掉，只留它前面的 |
   | bonus（`index == num_accepted`） | `num_accepted` | `num_accepted` | 它从来没作为输入行进过模型，没有 KV 可退 |

   也就是说 `index == num_accepted` 那一支正好等于初值，不需要单独判断。
