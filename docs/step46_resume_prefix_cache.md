...
**为什么这个条件里要带 `preemption_mode == "recompute"`？**

因为去掉它**会真的改变 legacy 的行为**。两种发布时机下，命中的是**不同状态**的块：

| 发布时机 | 命中时被命中块的状态 |
|---|---|
| step45（只在请求结束时发布） | 属主已经结束，`block_usage == 0`，是闲置缓存 |
| 每步发布（统一条件） | 可能是**正在运行**的请求的块，`block_usage >= 1` |

差别落在准入上：`_available_blocks(exclude=set(matched_blocks))` 里那个 `exclude`，
只对「本来会被算成可淘汰」的闲置块生效。命中闲置块时它把 `m` 块从可用量里减掉，
与 `need = worst_case - m` 的减少**相互抵消**，准入严格度不变；命中**运行中**的块时，
那块本来就不在可淘汰集合里，`exclude` 是空操作，于是 `need` 白白小了 `m` —— **准入变松**。

实测（`legacy`，`max_num_seqs=2`、池子 5 块、`block_size=4`）：

```text
A：prompt 4（1 块）、最多生成 8 -> worst_case = 3 块
B：prompt 8（与 A 共享第 0 块）、最多生成 4 -> worst_case = 3 块，delay 1 步到达

step45 时机：A 算完 prompt 后第 0 块**不发布**，B 命中 0 块
             free 4 + evictable 0 - promised 2 = 2 < need 3  -> B 继续等
每步发布：  第 0 块已发布且 block_usage = 1，B 命中 1 块
             need 3-1 = 2 <= 2                          -> B 立刻准入
```

结果：B 提前进入，**完成顺序从 `A,B` 变成 `B,A`**。在 768 组参数扫描
（池子 5~8 块 × 并发 2~3 × prompt 长度 × 生成长度 × 到达延迟）里有 **108 组完成顺序或输出不同**。

所以这不是「保守起见」，而是需求 §4.8「旧三种配置与 step45 行为一致」的**硬要求**。
（早先一版文档只跑了 135 组、没构造出「命中运行中块」的场景，得出了「测不出差异」的错误结论——
扫描面必须覆盖到准入真正吃紧的配置才看得出来。）
