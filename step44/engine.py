...
def _check_preemption_mode(preemption_mode, enable_prefix_caching):
    ...
    if preemption_mode not in PREEMPTION_MODES:
        raise ValueError(f"未知的 preemption_mode: {preemption_mode!r}，"
                         f"可选 None（承诺式，第四十三关行为）或 'recompute'")
    if preemption_mode == "recompute" and enable_prefix_caching:
        # 两个一起开就必须选一个让路——不静默关掉其中一个
        raise ValueError("preemption_mode='recompute' 需要 enable_prefix_caching=False："
                         "重算式抢占会丢弃并重建 KV，与共享前缀块的引用计数语义冲突。"
                         "请显式传 enable_prefix_caching=False")
...
class Engine:
    ...
    def __init__(self, max_num_seqs=1, max_num_batched_tokens=4,block_size=4, num_kv_blocks=8, vocab_size=5, d_model=8, max_seq_len=32, on_finished=None, enable_prefix_caching=True, device=None, attention_backend="torch", use_cuda_graph=False, num_q_heads=1, num_kv_heads=1,
        ...
        _check_preemption_mode(preemption_mode, enable_prefix_caching)
        if model is None:
