...
def _check_speculative(speculative_mode, num_speculative_tokens, prompt_lookup_n,
    ...
    第五十三关放开了 `max_num_seqs=1`：一个 batch 里可以同时有多个投机请求、
    ...
    docs/step53_batched_speculative.md。
    """
...
class Engine:
    ...
    def _sample(self, logits, picked):
        ...
        if self.speculative_mode is None:
            # 没开投机时走原来的采样后端：随机采样、惩罚项都在这里，不能被裸 argmax 顶掉
            self._sample_with_sampler(logits, picked, notify)
            return None
        ...
        for item in picked:
            ...
            if item["draft_ids"]:
                # 投机项：K+1 枚里前 K 枚验证草稿、最后一枚是 bonus，验证完再回滚+提交
    ...
    def _sample_with_sampler(self, logits, picked, notify):
        ...
        for item, output_id in zip(picked, output_ids):
            self._commit_tokens(item["request"], [output_id], notify)
