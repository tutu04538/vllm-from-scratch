...
class TinyCausalLM(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor):
        ...
        position_ids = torch.arange(seq_len, device=input_ids.device).unsqueeze(0).expand(batch_size, seq_len).to(self.device)
...
class Engine:
    ...
    def __init__(self, max_num_seqs=2, vocab_size=1000, d_model=512, max_seq_len=512, on_finished=None):
